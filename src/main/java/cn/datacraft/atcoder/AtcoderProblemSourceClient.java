...
class AtcoderProblemSourceClient implements AtcoderProblemSourceGateway {
    ...
    public String fetchTaskPage(String contestId, String taskId) {
        ...
        HttpRequest request = HttpRequest.newBuilder(URI.create(TASK_URL.formatted(contestId, taskId)))
                ...
                .build();
        try {
            HttpResponse<String> response = http.send(request, responseInfo -> HttpResponse.BodySubscribers.mapping(
            ...
            if (response.statusCode() == 404) throw new IllegalArgumentException("AtCoder 题目不存在或尚未开放");
